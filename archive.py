...
def is_user_banned(user_id):
    return check_user_ban(user_id) is not None
...
# ==================== КОМАНДЫ БЭКАПА (ВСЕ ДОЛЖНЫ БЫТЬ ЗДЕСЬ) ====================
restore_sessions = {}
...
def secure_backup_command(message):
    ...
    try:
        ...
        bot.send_message(message.chat.id,
                        ...
                        "3. Отправь команду: /secure_restore\n"
                        "4. Пришли ВСЕ части по очереди\n\n"
...
def secure_restore_start_command(message):
    ...
    bot.send_message(message.chat.id,
                    ...
                    "3. После этого база восстановится\n\n"
    ...
    try:
        if session['step'] == 'waiting_count':
            ...
            if parts_count < 1 or parts_count > 100:
                bot.send_message(message.chat.id, "❌ Неверное количество частей (1-100)")
                del restore_sessions[user_id]
                return
            ...
            if received >= total:
                ...
                if decrypted is None:
                    bot.send_message(message.chat.id, 
                                    "❌ **ОШИБКА РАСШИФРОВКИ!**\n"
                                    "Неверный ключ или поврежденные данные")
                else:
                    ...
                    bot.send_message(message.chat.id,
                                    ...
                                    "🔐 Шифрование: AES-256\n"
                                    "📊 Размер: {:,} байт\n"
                                    "🔄 Перезапусти бота: /start".format(len(decrypted)))
        ...
        bot.send_message(message.chat.id, "❌ Отправь число (например: 5)")
    except Exception as e:
...
"""
    ...
    text += f"\n<b>📊 Всего активных тем:</b> {active_topics}"
...
<i>Попробуйте снова:</i>"""
    ...
    c = db.cursor()
    c.execute('SELECT user_id FROM user_names WHERE username = ? AND user_id != ?', (new_username, user_id))
    if c.fetchone():
        text = """❌ <b>ИМЯ УЖЕ ЗАНЯТО</b>

Это имя уже используется другим пользователем.
Попробуйте другое имя."""
    else:
        c.execute('INSERT OR REPLACE INTO user_names (user_id, username, updated_at) VALUES (?, ?, CURRENT_TIMESTAMP)', (user_id, new_username))
        db.commit()
