...
def init_db():
    ...
    c.execute('''
        CREATE TABLE IF NOT EXISTS user_stats (
            ...
            notifications_enabled BOOLEAN DEFAULT TRUE  -- НОВОЕ: статус уведомлений
        )
    ''')
...
def are_notifications_enabled(user_id):
    ...
    try:
        conn = get_db_connection()
        c = conn.cursor()
        c.execute('SELECT notifications_enabled FROM user_stats WHERE user_id = %s', (user_id,))
        result = c.fetchone()
        conn.close()
...
def toggle_notifications(user_id):
    ...
    try:
        ...
        if result:
            ...
            # Если записи нет, создаем с выключенными уведомлениями
            new_status = False
            c.execute('''
                INSERT INTO user_stats (user_id, topics_created, replies_written, replies_received, notifications_enabled)
                VALUES (%s, 0, 0, 0, %s)
            ''', (user_id, new_status))
...
def add_topic(text, user_id):
    ...
    try:
        ...
        c.execute('''
            ...
            ON CONFLICT (user_id) 
            DO UPDATE SET 
                topics_created = user_stats.topics_created + 1,
                last_active = CURRENT_TIMESTAMP
        ''', (user_id,))
        
        # 5. Увеличиваем счетчик дневного лимита
...
def add_reply(topic_id, text, user_id):
    ...
    try:
        ...
        c.execute('''
            ...
            ON CONFLICT (user_id) 
            DO UPDATE SET 
                replies_written = user_stats.replies_written + 1,
                last_active = CURRENT_TIMESTAMP
        ''', (user_id,))
        ...
        c.execute('''
            ...
            ON CONFLICT (user_id) 
            DO UPDATE SET 
                replies_received = user_stats.replies_received + 1
        ''', (topic_author_id,))
        
        conn.commit()
        conn.close()
        
        # 5. Отправляем уведомление (если не отвечает сам себе и уведомления включены)
        if topic_author_id != user_id and are_notifications_enabled(topic_author_id):
...
def get_random_topic(exclude_user_id=None, viewed_topics=None):
    ...
    if viewed_topics and len(viewed_topics) > 0:
        ...
        if exclude_user_id:
            c.execute('SELECT * FROM topics WHERE is_active = TRUE AND user_id != %s ORDER BY RANDOM() LIMIT 1', 
                     (exclude_user_id,))
        else:
            c.execute('SELECT * FROM topics WHERE is_active = TRUE ORDER BY RANDOM() LIMIT 1')
...
def get_user_statistics(user_id):
    ...
    return {
        ...
        'notifications_enabled': stats[5] if len(stats) > 5 else True
    }
...
def get_top_users(limit=10):
    ...
    try:
        c.execute('''
            SELECT 
                us.user_id,
                COALESCE(un.username, 'user_' || us.user_id::text) as username,
                us.topics_created,
                us.replies_written,
...
def get_weekly_record():
    ...
    c.execute('''
        SELECT 
            ...
            COALESCE(un.username, 'user_' || t.user_id::text) as author_name
        FROM topics t
        ...
        GROUP BY t.id, t.text, un.username
...
def get_admin_statistics():
    ...
    try:
        ...
        # 2. АКТИВНЫЕ ПОЛЬЗОВАТЕЛИ за 24 часа
        c.execute('''
            SELECT COUNT(DISTINCT user_id) FROM (
        ...
        # 3. НОВЫЕ ПОЛЬЗОВАТЕЛИ за 24 часа
        c.execute('''
            SELECT COUNT(DISTINCT user_id) FROM (
...
📅 <i>Время: {datetime.now().strftime('%d.%m.%Y %H:%M')}</i>"""
        ...
        markup.add(
            ...
            telebot.types.InlineKeyboardButton("🔕 ВЫКЛ. УВЕДОМЛЕНИЯ", callback_data=f"toggle_notifications_{user_id}_from_notification")
        )
...
# ==================== КОМАНДА /TOP (РАБОТАЕТ ВО ВСЕХ ЧАТАХ) ====================
@bot.message_handler(commands=['top'])
def top_command(message):
    """Обработка команды /top - топ пользователей (работает во всех чатах)"""
...
"""
    ...
    # Удаляем команду /top (только в личных чатах)
...
• Создано сегодня: {topics_today}/{DAILY_TOPIC_LIMIT} тем"""
    ...
    if notifications_enabled:
        markup.add(telebot.types.InlineKeyboardButton("🔕 ВЫКЛЮЧИТЬ УВЕДОМЛЕНИЯ", callback_data="toggle_notifications"))
    else:
        markup.add(telebot.types.InlineKeyboardButton("🔔 ВКЛЮЧИТЬ УВЕДОМЛЕНИЯ", callback_data="toggle_notifications"))
...
def toggle_notifications_callback(call):
    """Переключение уведомлений в профиле"""
    user_id = call.from_user.id
    
    # Переключаем уведомления
    new_status, status_text = toggle_notifications(user_id)
    
    # Показываем сообщение об успехе
    if new_status:
        text = f"""✅ <b>УВЕДОМЛЕНИЯ ВКЛЮЧЕНЫ</b>

Теперь вы будете получать уведомления о новых ответах на ваши темы.

🔔 <b>Что включено:</b>
• Уведомления о новых ответах
• Информация об активности в ваших темах

<i>Уведомления можно отключить в любое время в вашем профиле.</i>"""
    else:
        text = f"""🔕 <b>УВЕДОМЛЕНИЯ ВЫКЛЮЧЕНЫ</b>

Вы больше не будете получать уведомления о новых ответах на ваши темы.

⚠️ <b>Что отключено:</b>
• Уведомления о новых ответах
• Оповещения об активности в ваших темах

<i>Уведомления можно включить в любое время в вашем профиле.</i>"""
...
@private_callback_only
def toggle_notifications_from_notification_callback(call):
    """Переключение уведомлений из уведомления о новом ответе"""
    try:
        ...
        # Проверяем, что пользователь переключает свои уведомления
        if user_id != target_user_id:
            bot.answer_callback_query(call.id, "❌ Это не ваши уведомления", show_alert=True)
            return
...
<i>Уведомления можно включить в любое время в вашем профиле.</i>"""
        ...
        logger.error(f"Ошибка в toggle_notifications_from_notification_callback: {e}")
...
def ignore_group_messages(message):
    """Игнорирование всех сообщений в групповых чатах (кроме /top)"""
...
if __name__ == '__main__':
    ...
    logger.info(f"📂 База данных PostgreSQL: {DATABASE_URL[:50]}...")
    logger.info("👤 Система уникальных имен 'аноним_XXXX' активирована")
    logger.info("🔔 Система уведомлений активирована (вкл/выкл)")
    ...
    logger.info("🏆 Команда /top активирована (работает во всех чатах)")
